...
PlayerbotPaladinAI::PlayerbotPaladinAI(Player* const master, Player* const bot, PlayerbotAI* const ai) : PlayerbotClassAI(master, bot, ai)
{
	...
	if (spec == PALADIN_SPEC_HOLY) {
		...
		m_statWeights[ITEM_MOD_SPIRIT] = 0.15f;
...
bool PlayerbotPaladinAI::IsNewItemAnUpgrade(Item const *pNewItem, Item const *pCurrentItem)
{
	...
	if (pNewItem->GetProto()->Class == ITEM_CLASS_WEAPON && pCurrentItem->GetProto()->Class == ITEM_CLASS_WEAPON) {
		...
		if (spec == PALADIN_SPEC_PROTECTION) {
			...
			float newArmor = pNewItem->GetProto()->Armor / 20;
			float currentArmor = pCurrentItem->GetProto()->Armor / 20;
